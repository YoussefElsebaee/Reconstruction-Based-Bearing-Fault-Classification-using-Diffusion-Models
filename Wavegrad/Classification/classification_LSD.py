    ...
    for folder in os.listdir(test_data_dir):
        ...
        for wav_file in tqdm(wav_files, desc=f"Processing {folder}"):
            ...
            for idx, cls_name in enumerate(class_names):
                ...
                LSD_scores[idx] = log_spectral_distance(original_waveform, g_wave)

            predicted_class = min(LSD_scores, key=LSD_scores.get)

            results.append({
                "sample": wav_file,
                "true_class": true_class,
                "predicted_class": predicted_class,
                **{f"LSD_{name}": LSD_scores[i] for i, name in enumerate(class_names)}
            })
...
if __name__ == "__main__":
    ...
    #save_dir= r"C:\Users\yosef\OneDrive - GIU AS - German International University of Applied Sciences\Desktop\metrics (LSD)\0% noise"
    df, cm, overall_acc = classify(test_data_dir, models)

# Save classification results
df.to_excel("classification_results_LSD.xlsx", index=False)
